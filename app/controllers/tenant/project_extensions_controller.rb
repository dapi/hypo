module Tenant
  class ProjectExtensionsController < ApplicationController
    ...
    def index
      render locals: { project_extensions: current_account.project_extensions.joins(:extension, :blockchain).order(:created_at) }
    end
    ...
    def update
      ...
      redirect_to tenant_project_extension_path(project_extension), notice: "Изменилось расширение #{project_extension.title}"
    ...
    def create
      ...
      redirect_to tenant_project_extension_path(project_extension), notice: "Создано расширение #{project_extension.title}"
    rescue ActiveRecord::RecordInvalid => e
    ...
    def permitted_params
      ...
      params.
        ...
        permit(:title, :extension_id, :blockchain_id, :params, :params_string, :summary, :extra_dataset_paths, :extra_dataset_paths_list)
    end
  end
end
